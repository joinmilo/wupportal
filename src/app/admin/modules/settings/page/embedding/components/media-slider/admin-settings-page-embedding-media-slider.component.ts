...
@Component({
  selector: 'app-admin-settings-page-embedding-media-slider',
  templateUrl: './admin-settings-page-embedding-media-slider.component.html',
  styleUrls: ['./admin-settings-page-embedding-media-slider.component.scss'],
})
export class AdminSettingsPageEmbeddingMediaSliderComponent implements OnInit {
  ...
  private patchMediaValue(): void {
    ...
    ?.references?.map(reference => reference?.media);
  ...
  private createMediaValue(): Maybe<PageAttributeEntity> {
    ...
    return {
      references: this.form.value.media?.map(media => ({
        media
      })),
      type: {
        id: attribute?.type?.id,
